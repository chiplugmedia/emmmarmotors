...
$title = $price = $daily = $duration = $imageooprr = $description = $genMsg = $investmentMsg = "";
...
if (isset($_POST['addinvestment'])) {
    ...
    if (empty($title)) {
        ...
        if (in_array($imageType, $allowed) && $imageSize <= $maxSize) {
            ...
            $sql = $link->prepare("INSERT INTO investment_plans (title, price,description, daily, image, level , duration, status, reference, date) VALUES (?, ?,?, ?, ?,?, ?, ?, ?, ?)");
            $sql->bind_param("ssssssssss", $title, $price,$description, $daily, $newImgName, $level, $duration, $status,$reference, $created_at);

            if ($sql->execute()) {
                $path = $_SERVER['DOCUMENT_ROOT'] . "$stream/dashbord/img/invest/$newImgName";
